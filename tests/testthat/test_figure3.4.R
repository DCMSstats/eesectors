context("figure3.4 works as expected")

test_that(
  "figure3.4 runs without errors",
  {

    gva <- year_sector_data(GVA_by_sector_2016)

    expect_silent(figure3.4(gva))
...
test_that(
  "figure3.4 produces expected plot",
  {

    # Check whether comparison file exists, and delete if so

    test_png <- file.path("figures", "test_figure3.4_test.png")
    ref_png <- file.path("figures", "test_figure3.4_reference.png")
    ...
    ggplot2::ggsave(
      width = 5,
      height = 5,
      filename = "figures/test_figure3.4_test.png"
    )
    ...
    visualTest::isSimilar(
      file = "figures/test_figure3.4_reference.png",
      fingerprint = visualTest::getFingerprint(
        file = "figures/test_figure3.4_test.png"
      ),
      threshold = 1e-3
    )
